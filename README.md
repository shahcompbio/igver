...
pip install igver
```
I would also suggest you pull the IGVer singularity container so it's saved to your cache, as follows. This is to ensure you're using an environment where both `xvfb-run` (required to redirect IGV GUI to an image) and IGV jar files are present.
```
