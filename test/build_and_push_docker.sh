#!/bin/bash
# Script to build and push IGVer Docker image to Docker Hub
...
log -e "${GREEN}Building IGVer Docker image...${NC}"
...
docker build --no-cache -t sahuno/igver:latest .
...
docker push sahuno/igver:${IGV_VERSION}

if [ ! -z "$1" ]; then
    docker push sahuno/igver:${VERSION}
